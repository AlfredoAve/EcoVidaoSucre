...
router.delete('/', authMiddleware, async (req, res) => {
  try {
    const eliminadas = await NotificacionesRepository.eliminarTodas(req.usuario.id);
    res.json({ exito: true, eliminadas });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    const eliminada = await NotificacionesRepository.eliminar(req.params.id, req.usuario.id);
    if (!eliminada) {
